...
class Editsess extends Component
{
    public  $session_id,
            $session,
            $allsessions,
    ...
    {
        return view('livewire.admin.editsess',[
    ...
    public function createSession()
    {
        ...
        // $testid = Session::where(['id'=>$this->sessid])->first();
        if($this->sessid){
            $this->session_id = $this->sessid;
    ...
    {
        $this->session = Session::find($this->session_id);
    }
        ...
        $this->getThisSessionContacts();
            ...
            foreach ($this->candidate_contacts as $key ) {
                ...
                if($this->session_type === 0){
                    ...
                    $message = "شما به جلسه ".$this->name."دعوت شدید "."لینک شما :".$this->video_link;
                }else{
                    $message = "شما به جلسه ".$this->name."دعوت شدید "."لینک شما :".$this->video_link ." کد احراز هویت شما: ".$key->token;
    ...
    {
        if(session_contact::where('s_id',$this->session_id)->get()){
